...
namespace simplebutton {
    ...
    void AnalogStick::setup(uint8_t xPin, uint8_t yPin, uint8_t buttonPin) {
        ...
        button = new ButtonPullup(buttonPin);
        up = new ButtonAnalog(yPin);
        down = new ButtonAnalog(yPin);
        left = new ButtonAnalog(xPin);
        right = new ButtonAnalog(xPin);
    ...
    void AnalogStick::setLogic(uint16_t logic, uint8_t tolerance) {
        ...
        uint16_t difference = static_cast<uint16_t>((double)logic * ((double)tolerance / double(100)));
