...
class Names {
    ...
    private:
        ...
        struct Device {
            uint8_t* mac;      // MAC-адрес
            char   * name;     // Имя сохраненного устройства
            uint8_t* apBssid;  // MAC-адрес точки доступа (если сохраненное устройство - станция)
            uint8_t  ch;       // Wi-Fi канал устройства
            bool     selected; // Выбрать для атаки
        };
