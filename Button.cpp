...
void Button::updateEvents() {
    ...
    while (e != NULL) {
        switch (e->getMode()) {
            ...
            case e->MODE::CLICKED:
                if (clicked(e->getMinPushTime(), e->getMinReleaseTime())) e->run();
                break;

            case e->MODE::DOUBLECLICKED:
                if (doubleClicked(e->getMinPushTime(), e->getMinReleaseTime(), e->getTimeSpan())) e->run();
                break;

            case e->MODE::HOLDING:
                if (holding(e->getInterval())) e->run();
                break;
        }
        e = e->next;
    }
...
int Button::getPushTime() {
    return static_cast<int>(millis() - pushTime);
}
...
void Button::setOnPushed(ButtonEventFunction fnct) {
    this->addEvent(new PushEvent(fnct));
}

void Button::setOnReleased(ButtonEventFunction fnct) {
    this->addEvent(new ReleaseEvent(fnct));
}

void Button::setOnClicked(ButtonEventFunction fnct) {
    setOnClicked(fnct, defaultMinPushTime, defaultMinReleaseTime);
}

void Button::setOnClicked(ButtonEventFunction fnct, uint32_t minPushTime) {
    setOnClicked(fnct, minPushTime, defaultMinReleaseTime);
}

void Button::setOnClicked(ButtonEventFunction fnct, uint32_t minPushTime, uint32_t minReleaseTime) {
    this->addEvent(new ClickEvent(fnct, minPushTime, minReleaseTime));
}

void Button::setOnDoubleClicked(ButtonEventFunction fnct) {
    setOnDoubleClicked(fnct, defaultMinPushTime, defaultMinReleaseTime, defaultTimeSpan);
}

void Button::setOnDoubleClicked(ButtonEventFunction fnct, uint32_t minPushTime) {
    setOnDoubleClicked(fnct, minPushTime, defaultMinReleaseTime, defaultTimeSpan);
}

void Button::setOnDoubleClicked(ButtonEventFunction fnct, uint32_t minPushTime, uint32_t timeSpan) {
    setOnDoubleClicked(fnct, minPushTime, defaultMinReleaseTime, timeSpan);
}

void Button::setOnDoubleClicked(ButtonEventFunction fnct, uint32_t minPushTime, uint32_t minReleaseTime, uint32_t timeSpan) {
    this->addEvent(new DoubleclickEvent(fnct, minPushTime, minReleaseTime, timeSpan));
}

void Button::setOnHolding(ButtonEventFunction fnct) {
    setOnHolding(fnct, defaultHoldInterval);
}

void Button::setOnHolding(ButtonEventFunction fnct, uint32_t interval) {
