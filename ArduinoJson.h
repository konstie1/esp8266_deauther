...
union JsonVariantContent {
  ...
  JsonUInt asInteger;
  const char *asString;
  class JsonArray *asArray;
  class JsonObject *asObject;
};
...
template <>
struct JsonVariantAs<class JsonArray> {
  typedef class JsonArray &type;
};

template <>
struct JsonVariantAs<const class JsonArray> {
  typedef const class JsonArray &type;
};

template <>
struct JsonVariantAs<class JsonObject> {
  typedef class JsonObject &type;
};

template <>
struct JsonVariantAs<const class JsonObject> {
  typedef const class JsonObject &type;
};
...
class JsonVariantCasts {
 public:
  ...
  FORCE_INLINE class JsonArray &asArray() const {
    return impl()->template as<class JsonArray>();
  }

  DEPRECATED("use as<JsonObject>() instead")
  FORCE_INLINE class JsonObject &asObject() const {
    return impl()->template as<class JsonObject>();
  }
  ...
  FORCE_INLINE operator class JsonArray &() const {
    return impl()->template as<class JsonArray &>();
  }

  FORCE_INLINE operator class JsonObject &() const {
    return impl()->template as<class JsonObject &>();
  }
...
struct StringTraits<
    ...
    : StdStreamTraits {};
}  // простір імен Internals
}  // простір імен ArduinoJson
#endif
...
struct StdStringTraits {
  ...
  static duplicate_t duplicate(const TString& str, Buffer* buffer) {
    if (!str.c_str()) return NULL;  // <- Строка Arduino може повертати NULL
...
class IndentedPrint {
 ...
 private:
  ...
  static const int MAX_TAB_SIZE = 7;
};
...
struct FloatTraits<T, 8> {
  ...
  static const mantissa_type mantissa_max = (static_cast<mantissa_type>(1) << mantissa_bits) - 1;
...
class JsonArray : public Internals::JsonPrintable<JsonArray>,
 ...
 public:
  ...
  static JsonArray &invalid() {
    static JsonArray instance(nullptr);
    return instance;
  }
  template <typename T, size_t N>
...
namespace ArduinoJson {
  ...
  public:
    ...
    template <typename TString>
    Internals::JsonObjectSubscript<const TString&> operator[](const TString& key) {
    ...
    template <typename TString>
    const Internals::JsonObjectSubscript<const TString&> operator[](const TString& key) const {
      return Internals::JsonObjectSubscript<const TString&>(*const_cast<JsonObject*>(this), key);
    }

    template <typename TString>
    const Internals::JsonObjectSubscript<TString*> operator[](TString* key) const {
      return Internals::JsonObjectSubscript<TString*>(*const_cast<JsonObject*>(this), key);
    }
    ...
    template <typename TValue, typename TString>
    DEPRECATED("Second argument is not supported anymore")
    typename Internals::EnableIf<Internals::IsFloatingPoint<TValue>::value, bool>::type
    set(const TString& key, TValue value, uint8_t) {
      return set_impl<const TString&, const JsonVariant&>(key, JsonVariant(value));
    }
    ...
    template <typename TValue, typename TString>
    typename Internals::JsonVariantAs<TValue>::type get(const TString& key) const {
  ...
  private:
    ...
    typename Internals::JsonVariantAs<TValue>::type get_impl(TStringRef key) const {
      const_iterator it = findKey<TStringRef>(key);
      return it != end() ? it->value.as<TValue>() : Internals::JsonVariantDefault<TValue>::get();
    }
    ...
    bool set_impl(TStringRef key, TValueRef value) {
      ...
      if (it == end()) {
        ...
        if (it == end()) return false;
        bool key_ok = Internals::ValueSaver<TStringRef>::save(_buffer, it->key, key);
...
namespace ArduinoJson {
    ...
    inline const char *JsonVariant::variantAsString() const {
        using namespace Internals;
        if (_type == JSON_UNPARSED && _content.asString && !strcmp("null", _content.asString))
            return NULL;
    ...
    inline bool JsonVariant::variantIsBoolean() const {
        ...
        return !strcmp(_content.asString, "true") || !strcmp(_content.asString, "false");
    }
...
template <typename Writer>
inline void ArduinoJson::Internals::JsonSerializer<Writer>::serialize(const JsonArray& array, Writer& writer) {
    writer.beginArray();
...
template <typename Writer>
inline void ArduinoJson::Internals::JsonSerializer<Writer>::serialize(const JsonArraySubscript& arraySubscript, Writer& writer) {
    serialize(arraySubscript.as<JsonVariant>(), writer);
}

template <typename Writer>
inline void ArduinoJson::Internals::JsonSerializer<Writer>::serialize(const JsonObject& object, Writer& writer) {
    writer.beginObject();
...
template <typename Writer>
template <typename TKey>
inline void ArduinoJson::Internals::JsonSerializer<Writer>::serialize(const JsonObjectSubscript<TKey>& objectSubscript, Writer& writer) {
    serialize(objectSubscript.template as<JsonVariant>(), writer);
}

template <typename Writer>
inline void ArduinoJson::Internals::JsonSerializer<Writer>::serialize(const JsonVariant& variant, Writer& writer) {
